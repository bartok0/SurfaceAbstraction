#include "pch.h"
#include "SurfaceAbstractionMain.h"
#include "Common\DirectXHelper.h"

#include <windows.graphics.directx.direct3d11.interop.h>

using namespace SurfaceAbstraction;
	...
	m_deviceResources(deviceResources)
{
	...
	m_gamepadAddedEventToken = Gamepad::GamepadAdded(bind(&SurfaceAbstractionMain::OnGamepadAdded, this, _1, _2));
	m_gamepadRemovedEventToken = Gamepad::GamepadRemoved(bind(&SurfaceAbstractionMain::OnGamepadRemoved, this, _1, _2));
	...
	if (m_canGetDefaultHolographicDisplay)
	{
		...
		m_holographicDisplayIsAvailableChangedEventToken = HolographicSpace::IsAvailableChanged(bind(&SurfaceAbstractionMain::OnHolographicDisplayIsAvailableChanged, this, _1, _2));
	}
...
void SurfaceAbstractionMain::SetHolographicSpace(HolographicSpace const& holographicSpace)
{
	...
	m_cameraAddedToken = m_holographicSpace.CameraAdded(std::bind(&SurfaceAbstractionMain::OnCameraAdded, this, _1, _2));
	...
	m_cameraRemovedToken = m_holographicSpace.CameraRemoved(std::bind(&SurfaceAbstractionMain::OnCameraRemoved, this, _1, _2));
...
void SurfaceAbstractionMain::UnregisterHolographicEventHandlers()
{
...
SurfaceAbstractionMain::~SurfaceAbstractionMain()
{
...
HolographicFrame SurfaceAbstractionMain::Update()
{
	...
	if (surfaceObserver == nullptr) {
		if (!surfaceAccessRequested) {
			...
			concurrency::task<winrt::Windows::Perception::Spatial::SpatialPerceptionAccessStatus> initObserverTask;
	...
	if (surfaceAccessAllowed) {
		...
		if (surfaceObserver == nullptr) {
			...
			//create observer
			surfaceObserver = winrt::Windows::Perception::Spatial::Surfaces::SpatialSurfaceObserver();
			if (surfaceObserver) {
				surfaceObserver.SetBoundingVolume(boudingVolume);

				auto mapContainingSurfaceCollection = surfaceObserver.GetObservedSurfaces();
		...
		//Keep observer positioned at headsets location
		surfaceObserver.SetBoundingVolume(boudingVolume);
	}
	//---
...
bool SurfaceAbstractionMain::Render(HolographicFrame const& holographicFrame)
{
...
void SurfaceAbstractionMain::SaveAppState()
{
	//
...
void SurfaceAbstractionMain::LoadAppState()
{
	//
...
void SurfaceAbstractionMain::OnPointerPressed()
{
	m_pointerPressed = true;
}
...
void SurfaceAbstractionMain::OnDeviceLost()
{
#ifdef DRAW_SAMPLE_CONTENT
...
void SurfaceAbstractionMain::OnDeviceRestored()
{
#ifdef DRAW_SAMPLE_CONTENT
...
void SurfaceAbstractionMain::OnLocatabilityChanged(SpatialLocator const& sender, winrt::Windows::Foundation::IInspectable const& args)
{
...
void SurfaceAbstractionMain::OnCameraAdded(
...
void SurfaceAbstractionMain::OnCameraRemoved(
	HolographicSpace const& sender,
...
void SurfaceAbstractionMain::OnGamepadAdded(winrt::Windows::Foundation::IInspectable, Gamepad const& args)
{
...
void SurfaceAbstractionMain::OnGamepadRemoved(winrt::Windows::Foundation::IInspectable, Gamepad const& args)
{
...
void SurfaceAbstractionMain::OnHolographicDisplayIsAvailableChanged(winrt::Windows::Foundation::IInspectable, winrt::Windows::Foundation::IInspectable)
{
	...
	if (m_spatialLocator != spatialLocator)
	{
		...
		if (spatialLocator != nullptr)
		{
			...
			m_locatabilityChangedToken = m_spatialLocator.LocatabilityChanged(std::bind(&SurfaceAbstractionMain::OnLocatabilityChanged, this, _1, _2));
